...
class FedSGMAAggregator(object):
    ...
    def aggregate(self):
        ...
        param_init = self.get_global_model_params() 
        ...
        for key in averaged_params.keys():
            ...
            abs_mean_signs = torch.abs(mean_signs) - tau
            sigma_sq = torch.var(diff_params[key])/(torch.mean(diff_params[key]))**2 
            mask = torch.max(0,torch.tanh(abs_mean_signs/sigma_sq))
            averaged_params[key] = param_init[key] + torch.sum(diff_params[key]*mask, dim=0)
